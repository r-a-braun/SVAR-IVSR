...
function [Waldstat] = fstat_montiel(y,m,p,nwlag)
% %‘first-stage’ statistics testing the relevance condition required for identification.
% Montiel Olea et al. (2017) derive the F-statistic for the null hypothesis that E[mt?1t]\neq 0
% and show that its ‘center’ is analogous to the ‘concentration’ parameter in the linear IV
% model. This F-statistic provides an indication of possible weak-instrument concerns for
% inference, with the 5% critical value of 3.84, and the Stock and Yogo (2005) threshold
% value of 10 as useful reference points
Y = y(p+1:end,:);
...
[ ~,What,~] = CovAhat_Sigmahat_Gamma( p,X, m, res',nwlag);
Waldstat= ((( T^.5)*Gamma(1,1))^2)/What(((n^2)*p)+1,((n^2)*p)+1);
